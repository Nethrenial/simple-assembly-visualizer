...
export const instructions: { [key: string]: Function } = {
  ...
  load: (opr: [string, string]) => {
    reg.value[opr[0]] = memory.value[Number.parseInt(opr[1])] as number;
    reg.value["pc"] = (reg.value["pc"] as number) + 1;
  },
  loadr: (opr: [string, string]) => {
    reg.value[opr[0]] = memory.value[reg.value[opr[1]] as number] as number;
    reg.value["pc"] = (reg.value["pc"] as number) + 1;
  },
  store: (opr: [string, string]) => {
    memory.value[Number.parseInt(opr[0])] = reg.value[opr[1]] as number;
    reg.value["pc"] = (reg.value["pc"] as number) + 1;
  },
  storer: (opr: [string, string]) => {
    memory.value[reg.value[opr[0]] as number] = reg.value[opr[1]] as number;
    reg.value["pc"] = (reg.value["pc"] as number) + 1;
  },
  ...
  call: (opr: [string, string]) => {
    ...
    memory.value[reg.value["sp"] as number] = (reg.value["pc"] as number) + 1;
    reg.value["pc"] = Number.parseInt(opr[0]);
  },
  ret: () => {
    reg.value["pc"] = memory.value[reg.value["sp"] as number] as number;
    reg.value["sp"] = (reg.value["sp"] as number) - 1;
  },
  out: (opr: [string, string]) => {
    console.log(reg.value[opr[0]]);
    outputs.value.push(reg.value[opr[0]] as number);
    reg.value["pc"] = (reg.value["pc"] as number) + 1;
  },
  push: (opr: [string, string]) => {
    reg.value["sp"] = (reg.value["sp"] as number) + 1;

    memory.value[reg.value["sp"] as number] = reg.value[opr[0]] as number;
    reg.value["pc"] = (reg.value["pc"] as number) + 1;
  },
  pop: (opr: [string, string]) => {
    reg.value[opr[0]] = memory.value[reg.value["sp"] as number] as number;
    reg.value["sp"] = (reg.value["sp"] as number) - 1;
